...
const agreeTnc = async (req, res) => {
    try {
        await agreeTncForAffiliate(req.affiliateUser.id);
        res.status(200).json({ status: "success", message: "Terms and conditions accepted" });
    } catch (error) {
        res.status(400).json({ status: "error", message: error.message });
    }
};

export default { login, changePassword, forgotPassword, agreeTnc };
