...
const RelatedPostCard = ({ data }: { data: any }) => {
  const blog = data? data.sort(
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {blog.map((d: any, i: number) => (
          <div key={`related-image-${i + 1}`} className="flex flex-col h-full w-full rounded-2xl border-[1px] border-gray-300 shadow-sm hover:shadow-lg transition-all">
            <Image
              src={`${process.env.NEXT_PUBLIC_BASE_URL}/uploads/${d.image}`}
              alt={`${d.title} image`}
              className="w-[80%] h-[80%] self-center object-contain rounded-t-2xl"
              width={1920}
              height={1080}
            />
            <div className="w-full border-t-[1px] border-gray-400 bg-white flex flex-col h-1/2 rounded-b-2xl justify-center gap-4 p-6">
              <Link href={`/insights/${d.id}`} className="hover:underline duration-500 transition-all text-lg lg:text-xl font-semibold text-primary line-clamp-2 font-heading !leading-[120%]">
                {d.title}
              </Link>
              <BlogContent content={d.content} className="text-sm lg:text-base line-clamp-3 text-gray-600 !leading-[150%]" />
            </div>
          </div>
        ))}
      </section>
