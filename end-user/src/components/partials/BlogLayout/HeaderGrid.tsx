...
export default function BlogCard({ data, large = false, length }: BlogCardProps) {
    if (!data) return null;
    console.log(length)
    return (
        <Link
        ...
        >
            ...
            {large ? (
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent text-white flex-col flex gap-5 p-5 lg:p-12 justify-end">
                    ...
                    <BlogContent content={data.blog.content} className='line-clamp-2 lg:text-base text-sm text-white/60' />
                </div>
            ) : (
                <div className="absolute bottom-0 left-0 w-full h-fit bg-white text-black flex flex-col gap-3 p-5 lg:py-5 lg:px-6 justify-end z-20">
                    ...
                    {/* <BlogContent content={data.blog.content} className='line-clamp-2 text-sm'/> */}
