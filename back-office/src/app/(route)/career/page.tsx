...
export default function DataPage() {
  ...
  const headings = ["Name", "Email", "Phone Number", "Action"];
  const data = career?.data.map((item: any) => ({
    "Name": item.name,
    "Email": item.email,
    "Phone Number": item.phoneNumber,
    "Action": (
      <div className="flex items-center gap-5">
        <Link href={`/career/edit?id=${item.id}`} className="text-blue-500">
  ...
  return (
    <main className="w-full flex flex-col gap-12">
      <Header title={"Career"} label={"Career Management"} />
      <section className="flex flex-col gap-16 p-8 rounded-3xl bg-white border border-border shadow-sm w-full min-h-[50vh] items-center">
        <div className="w-full flex justify-between items-center">
          <div className="flex flex-col gap-1 text-sm text-gray-600 justify-start w-full">
            <h1 className="text-4xl font-semibold text-black">All Career</h1>
            <p>showing all career</p>
          </div>
